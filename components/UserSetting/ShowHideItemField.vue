...
export default {
  ...
  mounted() {
    let _this = this
    let customItemFieldSetting = _this.$store.state.customItemFieldSetting
    customItemFieldSetting = JSON.parse(JSON.stringify(customItemFieldSetting))

    customItemFieldSetting.forEach(function(val) {
      if (val.show) {
  ...
  methods: {
    ...
    async submit() {
      ...
      await axios.post(process.env.NUXT_ENV_API_URL + "/set_member_setting", qs.stringify({
        UserID: userId,
        Token: token,
        UserFieldData: JSON.stringify(result),
      }))
      .then(response => {
        _this.$alert("修改成功")
        location.reload()
